...
class Search_3toursPage:
    ...
    def input_departure_city(self, Откуда='Ростов-на-Дону'):
        # (НЕГАТИВНЫЙ) ЧЕРЕЗ ВВОД ТЕКСТА В ПОЛЕ "Откуда"
        # города, которого нет в списке городов например Ростов-на-Дону
        # Тестовые данные ввода по полям:
        # Откуда = 'Ростов-на-Дону'  # (не валидный)Ростов-на-Дону
        # Куда = 'Турция' (валидный)
        # Дата_вылета = '10 декабря' (валидный)
        # Длительность = '10 ночей' (валидный)
        # Кто_едет = "2 взрослых" без детей (по умолчанию)(валидный)
        """
        ...
        :param: __driver: int время задержки в секундах
        """
        field = (WebDriverWait(self.__driver, 15).until(
            EC.visibility_of_element_located((
                By.XPATH, '//*[@id="app"]/div/div[1]/div/div/div/'
                          'div[1]/div/div/div[1]/div/div/input'))))
        field.click()
        # Попытка очистить как можно более надёжно:
        # field.clear()                   # Стандартный метод
        field.send_keys(Keys.CONTROL, 'a')  # Выделяем всё
        field.send_keys(Keys.BACKSPACE)     # Удаляем выделенное
        field.send_keys(Откуда)            # Вводим страну

        # Проверка результата
        validation_xpath = ('//*[@id="app"]/div/div[1]/div/div/div/div[1]/'
                            'div[1]/div/div[1]/div/div[2]/div/div')
        try:
            input_validation = WebDriverWait(self.__driver, 10).until(
                EC.visibility_of_element_located((By.XPATH, validation_xpath))
            )
            element_text = input_validation.text
            assert element_text == "Нет совпадений"
        except AssertionError:
            print("Введённый город найден или текст отличается.")

        print('Город поля "Откуда" введён')

    @allure.step("Ввод данных поиска поля 'Куда'")
    def input_arrival_country(self, country='Египет'):
        """
        ...
        field.send_keys(Keys.CONTROL, 'a')  # Выделяем всё
        field.send_keys(Keys.BACKSPACE)     # Удаляем выделенное
        field.send_keys(country)            # Вводим страну
